...
func answer(name, server string, dns *dnsMsg, qtype uint16) (cname string, addrs []dnsRR, err error) {
	...
	if dns.rcode == dnsRcodeNameError && dns.recursion_available {
		return "", nil, &net.DNSError{Err: errNoSuchHost.Error(), Name: name, Server: server}
	}
	if dns.rcode != dnsRcodeSuccess {
		...
		return "", nil, &net.DNSError{Err: "server misbehaving", Name: name, Server: server}
	}
...
Cname:
	...
	return "", nil, &net.DNSError{Err: "too many redirects", Name: name, Server: server}
}
